...
GA_MEASUREMENT_ID = os.environ.get("UCHUCHU_GA_ID", "G-N2EDF9P7P6")
...
# 調達情報（官公需情報ポータルサイト / 中小企業庁）— 無料の公開API
#
# 読者を「宇宙産業に参入したい製造業」と「サプライヤーを探す宇宙ベンチャー」に
# 絞ったとき、その両者が毎日確認する実務情報がこれ。ニュースは他媒体でも
# 読めるが、宇宙分野の調達だけを集めた一覧はどこにもない。
PROCUREMENT_API = "https://www.kkj.go.jp/api/"
# このサーバは誤った中間証明書を送るため、標準のCA束では検証に失敗する
# （macOSのcurlは中間を自動取得するので通るが、Pythonは通らない）。
# 検証を切らず、正しい中間とルートを同梱して通す。
KKJ_CA_BUNDLE = Path(__file__).resolve().parent / "certs" / "kkj-chain.pem"
# 複数の語で引いて重複排除する。1語だけでは取りこぼす（機関名では
# 案件名に宇宙と入らないものが拾えず、語だけでは他省庁の誤ヒットが増える）。
PROCUREMENT_QUERIES = [
    "宇宙航空研究開発機構", "人工衛星", "ロケット", "宇宙機", "宇宙開発",
]
# 案件名がこの語を含むものだけ残す。
# 概要まで見ると、造林事業や下水道工事の定型文にある「観測」「軌道」で
# 誤ヒットして一覧が使い物にならなくなる（実測で368件中9割が無関係だった）。
# 判定は案件名に限り、語も宇宙分野に固有のものだけに絞る。
PROCUREMENT_KEYWORDS = (
    "宇宙", "衛星", "ロケット", "打上", "打ち上げ", "探査機", "宇宙機",
    "JAXA", "軌道上", "深宇宙", "無重力", "微小重力",
)
# 宇宙分野の発注機関。案件名に語が無くても、ここからの発注は残す。
PROCUREMENT_ORGS = ("宇宙航空研究開発機構", "情報通信研究機構")
PROCUREMENT_LIMIT = 150

# 保持する最大件数（アーカイブ蓄積の上限）
#
# 2026-08-25: 600 / 350 は「上限」ではなく「削除装置」として働いていた。
# 収集元を増やしてニュースが1日50〜78件入るようになった結果、600件の枠は
# 11日分しか持たず、毎日およそ50〜70本の公開済みページが data から消え、
# サイト上で 404 になっていた。Google が7月に登録したページはほぼ全滅し、
# 8/18 から検索の表示が 41→4/日 に落ちた。Bing は落ちていないので
# 検索エンジン側の問題ではなく、これが原因。
#
# 一度公開したURLは消さない。増える分は一覧のページ送りが吸収する。
# 枠は「無制限にしない」ための安全弁として残すが、実データより十分大きく取る。
NEWS_LIMIT = 30000
PAPERS_LIMIT = 20000
LAUNCHES_LIMIT = 200
