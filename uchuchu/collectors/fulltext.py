...
# 生成する要約の目安。
# かつて下限600字だけを決めて上限を決めなかった結果、
# 元記事をほぼ全訳した1,000字前後の文章が生成され、
# 要約ではなく転載に近い状態になっていた。上限を必ず設ける。
TARGET_CHARS = 250         # 生成する要約の目安
MAX_CHARS = 300            # これを超えたら表示側で切り詰める
...
以下の記事本文を読み、日本語の短い紹介文を書いてください。

これは他媒体の記事を紹介するための文章です。
**元記事の代わりに読めるものを書いてはいけません。**
読者が「元記事を読むかどうか」を判断できる材料を示すのが目的です。

要件:
- **250字程度。300字を絶対に超えないこと**
- 1〜2段落。段落間は空行で区切る
- 「何が起きたか」を最初に述べる
- 記事の要点だけを示し、詳細・経緯・数字の列挙はしない
